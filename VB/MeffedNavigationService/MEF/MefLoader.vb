...
Namespace MeffedNavigationService.MEF
	...
	Public NotInheritable Class MEFImporter(Of T)
		Private privateImported As IEnumerable(Of Lazy(Of T, IModuleMetadata))
		<ImportMany(AllowRecomposition := True)> _
		Public Property Imported() As IEnumerable(Of Lazy(Of T, IModuleMetadata))
			Get
				Return privateImported
			End Get
			Set(ByVal value As IEnumerable(Of Lazy(Of T, IModuleMetadata)))
				privateImported = value
			End Set
		End Property
